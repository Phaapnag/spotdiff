...
ANSWERSECONDS = 2
MAX_DISPLAY = 1024
MAX_VIDEO_WIDTH = 720  # 限制影片寬度，避免 Render OOM
...
def make_video_with_opencv_frames(img1, img2, img2_marked, outpath):
    ...
    clip.write_videofile(outpath, codec="libx264", audio=False, preset="ultrafast")
...
def step2_make_video(base_full_state, variant_full_state, points, radius, thickness):
    ...
    if os.path.exists(base_path) and os.path.exists(variant_path):
        ...
        # 讀 State
        if base_full_state is None: raise gr.Error("請回到步驟 1 按『開始』。")
    ...
    # 畫圈（points 裡已有 r, t，所以這裡不用傳 radius/thickness）
...
with gr.Blocks(title="找不同 Shorts 生成器") as demo:
    ...
    with gr.Tab("步驟 1：上傳 & 對齊"):
        ...
        with gr.Row():
            # 設定相同高度讓它們看起來一樣大
            base_show = gr.Image(label="基準圖 (已對齊)", height=600)
            variant_show = gr.Image(label="變體圖 (點擊畫紅圈)", height=600, interactive=True)
    ...
    with gr.Tab("步驟 2：生成影片"):
        ...
        make_video_btn.click(step2_make_video, 
                             [base_full_state, variant_full_state, points_state, radius_slider, thickness_slider], 
                             video_out)
